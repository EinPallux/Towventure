...
Rule: **every enemy past floor 10 checks a build axis** (speed, burst, sustain, status, armor, gold). Stats/scaling in [BALANCE.md](BALANCE.md) §6. Anchor roster per biome (3 regulars + 1 elite each). **All ten biomes (floors 1–100) are authored in code** (`content/enemies.ts`, `content/biomes.ts`) as of Phase 4 — the Gatehouse through the Crown, ending on the Sleepless Warden. `✓` marks a check the sim models directly today; *(deferred)* marks a check that awaits vocabulary the sim/run layers don't have yet — the enemy still fights as an honest stat check meanwhile. Phase 4 added two golden-neutral enemy mechanics: `immuneToStatus` (Ember Courtier ← Burn) and enemy `hitsPerSwing` (Pressure Wraith's 5-tick anti-Ward blow). Biomes 6–10 authored a third thematic regular each (Vitrine Adder, Deadbolt Sentinel, Salon Shade, Court Duelist, Crown Sleeper) to hold the 3-regular quota; the doc's signature mirror/phase/linkage mechanics (true build-copy, Escrow's banked return, Curator's self-fight, Princess's Frost-flip, the Warden's three-phase Doomfall script) remain deferred with honest stat-check stand-ins.
...
*Implementation status (Phase 4, Slice B — lore expansion):* the Codex now carries authored lore for **all ten biomes' bosses + elites + regulars and every deep-tower Mythic** (biomes 6–10's 25 enemies and the nine Mythics — Sleepless Crown, Pale Candle, the Sleepless Eye, Aegis, the Long Way Down, the Hungering Coin, Heartpiercer, the Toll-Keeper's Bell, Mothlight Blade), in the melancholy never-explaining tone (ART_DIRECTION §8), on top of the Phase-2 anchor set.
