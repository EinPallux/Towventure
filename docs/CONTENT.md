...
*Implementation status (Phase 2):* the **event door type is live** — `generateDoors` turns ~1 in 5 regular floors' battle door into an event (never the only path forward), `chooseDoor` enters the `event` phase, and the `resolveEvent` command applies the chosen option (`content/events.ts` for copy, `run/events.ts` for the bespoke effect). Four anchors are wired: **Shrine of the Mended Blade** (upgrade a random item +1★, consume a material), **Sleepwalker's Bargain** (swap the two trinkets' ★), **Gambler's Alcove** (stake ¼ gold, double-or-nothing, seeded), and **Cursed Reliquary** (take a random Epic → reward screen; the next-boss Relic-copy curse is deferred). Option 0 is "accept", option 1 declines (a no-op that still resolves the floor). Events touching systems not yet built — marks (Tithe-Collector), mid-run Honor (The Beggar), Echoes (A Door Left Ajar), Sanctum (Fountain of Verdigris), relic-number reroll (An Honest Mirror), a run-persistent Max-HP tax (The Molting Wall) — are authored above and land with those systems.
